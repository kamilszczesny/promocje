 ...
 * @copyright 2007-2011
  ...
  **/
	 ...
	 * @package Internal/Operations
	 */
	class wi_WideImage_Operation_ResizeCanvas
	{
		/**
		...
		{
			$new_width = wi_WideImage_Coordinate::fix($width, $img->getWidth());
			$new_height = wi_WideImage_Coordinate::fix($height, $img->getHeight());
			...
			$new = wi_WideImage::createTrueColorImage($new_width, $new_height);
			if ($img->isTrueColor())
			{
				if ($color === null)
			...
			$x = wi_WideImage_Coordinate::fix($left, $new->getWidth(), $img->getWidth());
			$y = wi_WideImage_Coordinate::fix($top, $new->getHeight(), $img->getHeight());
