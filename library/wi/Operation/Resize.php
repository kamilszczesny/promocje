 ...
 * @copyright 2007-2011
  ...
  **/
	 ...
	 * @package Internal/Operations
	 */
	class wi_WideImage_Operation_InvalidFitMethodException extends wi_WideImage_Exception {}
	/**
	 * An Exception for when an invalid resize dimensions are passed
	 * 
	 * @package Internal/Operations
	 */
	class wi_WideImage_Operation_InvalidResizeDimensionException extends wi_WideImage_Exception {}
	 ...
	 * @package Internal/Operations
	 */
	class wi_WideImage_Operation_Resize
	{
		/**
		 * Prepares and corrects smart coordinates
		 *
		 * @param wi_WideImage_Image $img
		...
		protected function prepareDimensions($img, $width, $height, $fit)
		{
			...
			if ($width !== null)
				$width = wi_WideImage_Coordinate::fix($width, $img->getWidth());
			
			if ($height !== null)
				$height = wi_WideImage_Coordinate::fix($height, $img->getHeight());
			...
			elseif ($fit == 'inside' || $fit == 'outside')
			{
				...
				throw new wi_WideImage_Operation_InvalidFitMethodException("{$fit} is not a valid resize-fit method.");
		...
		function execute($img, $width, $height, $fit, $scale)
		{
			...
			if ($dim['width'] <= 0 || $dim['height'] <= 0)
				throw new wi_WideImage_Operation_InvalidResizeDimensionException("Both dimensions must be larger than 0.");
			
			if ($img->isTransparent() || $img instanceof wi_WideImage_PaletteImage)
			{
				$new = wi_WideImage_PaletteImage::create($dim['width'], $dim['height']);
				...
				if (!imagecopyresized(
						...
						$img->getWidth(), 
						$img->getHeight()))
					throw new wi_WideImage_GDFunctionResultException("imagecopyresized() returned false");
			}
			else
			{
				$new = wi_WideImage_TrueColorImage::create($dim['width'], $dim['height']);
				...
				if (!imagecopyresampled(
						...
						$img->getWidth(), 
						$img->getHeight()))
					throw new wi_WideImage_GDFunctionResultException("imagecopyresampled() returned false");
