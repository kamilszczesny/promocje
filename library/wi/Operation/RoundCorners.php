 ...
 * @copyright 2007-2011
  ...
  **/
	 ...
	 * @package Internal/Operations
	 */
	class wi_WideImage_Operation_RoundCorners
	{
		/**
		...
		function execute($image, $radius, $color, $smoothness, $corners)
		{
			...
			$corner = wi_WideImage::createTrueColorImage($radius * $sample_ratio, $radius * $sample_ratio);
			if ($color === null)
			{
				...
				if ($corners & wi_WideImage::SIDE_TOP_LEFT || $corners & wi_WideImage::SIDE_LEFT || $corners & wi_WideImage::SIDE_TOP)
					$result = $result->applyMask($corner, -1, -1);
				
				$corner = $corner->rotate(90);
				if ($corners & wi_WideImage::SIDE_TOP_RIGHT || $corners & wi_WideImage::SIDE_TOP || $corners & wi_WideImage::SIDE_RIGHT)
					$result = $result->applyMask($corner, $result->getWidth() - $corner->getWidth() + 1, -1, 100);
				
				$corner = $corner->rotate(90);
				if ($corners & wi_WideImage::SIDE_BOTTOM_RIGHT || $corners & wi_WideImage::SIDE_RIGHT || $corners & wi_WideImage::SIDE_BOTTOM)
					$result = $result->applyMask($corner, $result->getWidth() - $corner->getWidth() + 1, $result->getHeight() - $corner->getHeight() + 1, 100);
				
				$corner = $corner->rotate(90);
				if ($corners & wi_WideImage::SIDE_BOTTOM_LEFT || $corners & wi_WideImage::SIDE_LEFT || $corners & wi_WideImage::SIDE_BOTTOM)
