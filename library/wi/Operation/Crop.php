 ...
 * @copyright 2007-2011
  ...
  **/
	 ...
	 * @package Internal/Operations
	 */
	class wi_WideImage_Operation_Crop
	{
		/**
		...
		{
			$width = wi_WideImage_Coordinate::fix($width, $img->getWidth(), $width);
			$height = wi_WideImage_Coordinate::fix($height, $img->getHeight(), $height);
			$left = wi_WideImage_Coordinate::fix($left, $img->getWidth(), $width);
			$top = wi_WideImage_Coordinate::fix($top, $img->getHeight(), $height);
			if ($left < 0)
			{
				$width = $left + $width;
				$left = 0;
			...
			if ($width <= 0 || $height <= 0)
				throw new wi_WideImage_Exception("Can't crop outside of an image.");
