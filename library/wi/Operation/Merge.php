 ...
 * @copyright 2007-2011
  ...
  **/
	 ...
	 * @package Internal/Operations
	 */
	class wi_WideImage_Operation_Merge
	{
		/**
		 * Returns a merged image
		 *
		 * @param wi_WideImage_Image $base
		 * @param wi_WideImage_Image $overlay
		...
		{
			$x = wi_WideImage_Coordinate::fix($left, $base->getWidth(), $overlay->getWidth());
			$y = wi_WideImage_Coordinate::fix($top, $base->getHeight(), $overlay->getHeight());
			...
			if ($pct < 100)
			{
				if (!imagecopymerge(
					...
					$overlay->getHeight(), 
					$pct))
				throw new wi_WideImage_GDFunctionResultException("imagecopymerge() returned false");
			}
			else
			{
				if (!imagecopy(
