 ...
 * @copyright 2007-2011
  ...
  **/
	 ...
	 * @package Exceptions
	 */
	class wi_WideImage_NoFontException extends wi_WideImage_Exception {}
	
	/**
	 * @package Exceptions
	 */
	class wi_WideImage_InvalidFontFileException extends wi_WideImage_Exception {}
	
	/**
	 * @package Exceptions
	 */
	class wi_WideImage_InvalidCanvasMethodException extends wi_WideImage_Exception {}
	
	/**
	 * @package wi_WideImage
	 */
	class wi_WideImage_Canvas
	{
		protected $handle = 0;
		 ...
		 *
		 * Shouldn't be used directly, use wi_WideImage_Image::getCanvas() instead.
		 *
		 * @param wi_WideImage_Image $img Image object
		 */
		 ...
		 * wi_WideImage_Font_TTF, wi_WideImage_Font_PS, or wi_WideImage_Font_GDF.
		 *
		 *
		 *
		 *
		 ...
		 * @return mixed One of the wi_WideImage_Font_* objects
		 */
		function useFont($file, $size = 12, $color = 0, $bgcolor = null)
		{
			...
			if ($ext == 'ttf' || $ext == 'otf')
				$font = new wi_WideImage_Font_TTF($file, $size, $color);
			elseif ($ext == 'ps')
				$font = new wi_WideImage_Font_PS($file, $size, $color, $bgcolor);
			elseif ($ext == 'gdf')
				$font = new wi_WideImage_Font_GDF($file, $color);
			else
				throw new wi_WideImage_InvalidFontFileException("'$file' appears to be an invalid font file.");
		 ...
		 * 
		 * You must set a font with a call to wi_WideImage_Canvas::setFont() prior to writing text to the image.
		 * 
		...
		{
			if ($this->font === null)
				throw new wi_WideImage_NoFontException("Can't write text without a font.");
		...
		function __call($method, $params)
		{
			if (function_exists('image' . $method))
			{
				...
				throw new wi_WideImage_InvalidCanvasMethodException("Function doesn't exist: image{$method}.");
		}
	}
