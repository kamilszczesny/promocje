 ...
 * @copyright 2007-2011
  ...
  **/
	 ...
	 */
	class wi_WideImage_TrueColorImage extends wi_WideImage_Image
	{
		/**
		 * Creates the object
		 *
		...
		static function create($width, $height)
		{
			...
			return new wi_WideImage_TrueColorImage(imagecreatetruecolor($width, $height));
		}
		...
		function asPalette($nColors = 255, $dither = null, $matchPalette = true)
		{
			...
			$temp->releaseHandle();
			return new wi_WideImage_PaletteImage($temp->handle);
		}
		...
		function getChannels()
		{
			...
			if (count($args) == 1 && is_array($args[0]))
				$args = $args[0];
			return wi_WideImage_OperationFactory::get('CopyChannelsTrueColor')->execute($this, $args);
		}
		...
		function copyNoAlpha()
		{
			...
			$result = wi_WideImage_Image::loadFromString($this->asString('png'));
			$this->saveAlpha($prev);
