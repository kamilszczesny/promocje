<?php
class Form_RestaurantAdd extends Zend_Form 
{ 
    public function __construct($options = null, $cuisinetypes) 
    {
        if(empty($cuisinetypes)) $cuisinetypes = array();
        else{
          $cuisineConf = array();
          foreach($cuisinetypes as $key=>$item){
            $cuisineConf[$item['id']] = $item['typ_kuchni'];
          }
        }
       
        parent::__construct($options);
        $this->setName('add_restaurant');
        
        $this->setMethod('post');
        
        $cuisinetype = new Zend_Form_Element_Select('cuisinetype');
        $cuisinetype->setLabel('Wybór rodzaju kuchni')->setRequired(true);
        $cuisinetype->setMultiOptions($cuisineConf);
        ...
        $data_urodzenia = new Zend_Form_Element_Text(array('type'=>'date','name'=>'data_urodzenia'));
        ...
        $data_urodzenia->setLabel('Data urodzenia (RRR-MM-DD)')
        				->addValidator($validatorDate)
						->setAttrib('type','date');
		$data_urodzenia->helper = 'textField';
    ...
    //echo('</pre>');      
        ...
        $telefon->setLabel('Telefon')->setAttrib('type','tel');
		$telefon->helper = 'textField';;
              
        
        $submit = new Zend_Form_Element_Submit('submit');
        $submit->setLabel('zapisz');
        
        $this->addElements(array($imie, $nazwisko, $miasto, $ulica, $powiat, $wojewodztwo, $kod_pocztowy, $nr_domu, $nr_mieszkania, $data_urodzenia, $email, $telefon, $cuisinetype, /*$captcha,*/ $submit));
